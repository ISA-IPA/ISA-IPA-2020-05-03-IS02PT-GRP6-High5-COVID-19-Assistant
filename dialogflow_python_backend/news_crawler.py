...
def delete_news_data_db():
    ...
    db.commit()
    db.close()
    return 'All data in News Table have been deleted!'
...
def insert_db(data):
    ...
    tmp_list = [date_stamp, news_title, news_link, news_summary]
    query = "INSERT INTO NEWS_INFO (date_stamp, news_title, news_link, news_summary) VALUES (?,?,?,?);"

    db.execute(query, tmp_list)
    db.commit()
    db.close()
    return 'News has been successfully uploaded to DB'
